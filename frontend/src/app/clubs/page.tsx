...
const ClubCard = ({ data }: { data: Club }) => {
  ...
  return (
       <Dialog >
      <DialogTrigger className="flex flex-col text-center p-16 md:p-8 lg:p-16 hover:bg-gray-50 dark:bg-[#575c65]/35 dark:hover:bg-gray-600/55 rounded-lg transition-colors duration-200">
        <Image
          src={imgUrl}
          ...
          alt="Clubs"
        />
        <p className="mt-2 font-medium text-sm md:text-base text-gray-900 dark:text-gray-100">{data.name}</p>
      </DialogTrigger>
      <DialogContent className="w-[95vw] max-w-[1200px] h-[80vh] p-4 md:p-6 lg:p-8 overflow-y-auto bg-white dark:bg-black text-gray-900 dark:text-gray-100">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          <div className="flex justify-center items-center bg-gray-50 dark:bg-gray-600/40 rounded-lg p-4">
            <Image
              src={imgUrl}
          ...
          <div className="flex flex-col justify-between w-full h-full">
            <div className="space-y-3 md:space-y-4">
              <DialogTitle className="text-xl sm:text-2xl md:text-3xl lg:text-4xl text-center md:text-left font-bold text-gray-900 dark:text-gray-100">
                {data.name}
              </DialogTitle>
            ...
            <div className="flex flex-col gap-4 mt-4 md:mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
              <div className="flex flex-col items-start">
                <p className="font-semibold text-gray-600 dark:text-gray-400 text-sm md:text-base">
                  Faculty Coordinator
                </p>
                <div className="flex items-center gap-6">
                  <div className="flex items-center space-x-2">
                    ...
                    <Link href="data.faculty1url" target="_blank">
                      <ExternalLink
                        size={16}
                  ...
                  data.faculty2 !== "nil" && (
                    <div className="flex items-center space-x-2">
                      ...
                      <Link href="data.faculty2url" target="_blank">
                        <ExternalLink
                          size={16}
                          className="text-blue-600 dark:text-blue-400 cursor-pointer"
                        />
              ...
              {data.clubType && (
                <div className="flex flex-col items-start mt-2">
                  ...
                  <span className="inline-block bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-300 text-xs md:text-sm px-2.5 py-1 rounded-full mt-1">
                    {data.clubType}
                  </span>
                </div>
              )}
  ...
  return (
    <div>
      ...
      <div id="scroll-to-component" className="flex flex-col-reverse md:flex-row items-center w-[100%] h-[65%]">
        ...
        <Image
          src="/clubs/basketball.jpg"
          className="w-1/2 h-1/2 flex justify-center items-center mt-6 md:mt-0 md:h-[85vh] lg:w-[35%] object-cover"
          width={700}
          height={100}
      ...
      <div>
        <Heading className="text-center text-gray-900 dark:text-gray-100">Explore</Heading>
        <div className="flex space-x-8 text-[#9C9C9C] dark:text-gray-400 max-w-5xl mx-auto mt-8 px-4">
          <div className="flex w-[50%] relative justify-center items-center">
            <Input
              type="text"
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search"
              className="bg-[#EBEBEB] dark:bg-black text-black dark:text-gray-200 !text-xl pl-8 h-12 rounded-md "
            />
            <Search className="absolute left-2 bottom-1/3 text-gray-500 dark:text-gray-300 h-6 w-6" />
          </div>
          <div className="flex w-[50%] relative justify-center items-center text-[#9C9C9C] dark:text-gray-400">
            <Select
              onValueChange={(value) => {
                if (value === "all") {
                  setSearch("");
            ...
            >
              <SelectTrigger className="bg-[#EBEBEB] dark:bg-black dark:text-gray-200 w-[320px] py-6">
                <div className="flex space-x-2 items-center">
                  <ListFilter className="text-gray-500 dark:text-gray-300 h-6 w-6" />
                  <SelectValue placeholder="Select Club Type" className="text-lg" />
                </div>
              </SelectTrigger>
              <SelectContent className="bg-[#EBEBEB] dark:bg-black dark:text-gray-200">
                <SelectItem value="all" className="text-xl py-3">All Club Types</SelectItem>
                {clubTypes.map((type, index) => (
                  <SelectItem value={type} key={index} className="text-xl py-3">
                    {type}
                  </SelectItem>
                ))}
              </SelectContent>
